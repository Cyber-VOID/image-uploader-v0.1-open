                ...
                <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
                    <div class="sm:flex items-center justify-between">
                        ...
                        <button
                            ...
                            <a href="{{ route('images.new-index') }}"
                               class="text-sm font-medium leading-none text-white">New Image</a>
                        </button>
                    </div>
                            ...
                            @forelse($images as $image)
                                ...
                                <tr tabindex="0" class="focus:outline-none h-16 border border-gray-100 rounded">
                                    <td>
                                        <div class="ml-5">
                                            <div>
                                                <a href="{{ route('images.get',[$image->id,$image->token]) }}"
                                                   class="text-base font-medium leading-none text-gray-700 mr-2">{{ $image->id  }}</a>
                                    ...
                                    <td class="">
                                        <div class="flex items-center pl-5">
                                            <a href="{{ route('images.get',[$image->id,$image->token]) }}"
                                               class="text-base font-medium leading-none text-gray-700 mr-2">{{ $image->name  }}</a>
                                    ...
                                    <td class="pl-5">
                                        <div class="flex items-center">
                                            @if(isset($image->collection))
                                                <a href="{{route('images.get',[$image->collection->id,$image->collection->token])}}" class="text-sm leading-none text-gray-600 ml-2"></a>
                                            @else
                                                <p class="text-sm leading-none text-gray-600 ml-2">{{$image->name}}</p>
                                            @endif
                                    ...
                                    <td class="pl-4">
                                        <a type="submit"
                                           href="{{ route('url-generator.new-url', ['image',$image->id,$image->token]) }}"
                                           class="focus:ring-2 focus:ring-offset-2 focus:ring-red-300 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none">
                                            Generate Url
                                        </a>
